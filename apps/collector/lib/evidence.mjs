...
export function analyzeBrowser(data){
 ...
 const native=unique.filter(x=>x.record.source==='external'&&x.record.proxy);
 const emissions=unique.flatMap(x=>{try{const event=JSON.parse(x.record.message);return event.kind==='workshop-cookie-v1'?[{...x,event}]:[];}catch{return[];}});
 const rows=data.consumed.sort((a,b)=>a.at-b.at||a.slot-b.slot).map((step,index)=>{
  const es=emissions.filter(x=>x.event.eventId===step.eventId&&x.event.slot===step.slot);
  const ns=es.length===1?native.filter(x=>x.record.requestId===es[0].record.requestId):[];
  const joined=es.length===1&&ns.length===1&&!!ns[0].record.requestId;
  const wanted={A:['valid','fixture-A'],B:['valid','fixture-B'],Missing:['missing',null],Invalid:['invalid',null]}[step.expected];
  const observed=es[0]?.event;
  const cookieCorrect=!!observed&&observed.cookieState===wanted[0]&&observed.cookie===wanted[1];
  const cache=ns[0]?.record.proxy.vercelCache||null,expectedCache=index===0?'MISS':'HIT';
  const safe=!!ns[0]&&ns[0].record.nativeCookieFieldPresent===false&&[...ns,...es].every(x=>x.record.excludedValuePresent===false);
  return {slot:step.slot,name:step.expected,eventId:step.eventId,at:step.at,expectedCache,nativeCache:cache,nativeCount:ns.length,emissionCount:es.length,joined,nativeRequestId:ns[0]?.record.requestId||null,envelopeRequestId:es[0]?.record.requestId||null,nativeUserAgent:ns[0]?.record.proxy.userAgent||null,nativeReferrer:ns[0]?.record.proxy.referer||null,cookieState:observed?.cookieState||null,cookie:observed?.cookie??null,cookieCorrect,status:!joined?'PENDING':cookieCorrect&&cache===expectedCache&&safe?'PASS':'FAIL',delayMs:ns[0]?ns[0].receivedAt-ns[0].record.timestamp:null};
 });
 const allJoined=rows.length>0&&rows.every(r=>r.joined),covered=['A','B','Missing','Invalid'].every(n=>rows.some(r=>r.name===n&&r.status==='PASS'));
 const originCorrect=data.origins.length===1;
 const checks={cache:allJoined&&rows.every(r=>r.nativeCache===r.expectedCache),origin:originCorrect,cookies:allJoined&&rows.every(r=>r.cookieCorrect),join:allJoined,currentRequest:allJoined&&new Set(rows.map(r=>r.nativeRequestId)).size===rows.length,nativeCookieAbsent:native.length>0&&native.every(x=>x.record.nativeCookieFieldPresent===false),noUnrelatedCookie:all.length>0&&all.every(x=>x.record.excludedValuePresent===false)};
 const failed=rows.some(r=>r.status==='FAIL')||data.origins.length>1;
 return {mode:'browser',status:failed?'FAIL':!rows.length?'EMPTY':!allJoined?'PENDING':covered&&Object.values(checks).every(Boolean)?'PASS':'PARTIAL',checks,rows,coveredCases:[...new Set(rows.filter(r=>r.status==='PASS').map(r=>r.name))],originCount:data.origins.length,nativeCount:native.length,emissionCount:emissions.length,duplicateRecords:all.length-unique.length,unmatchedEmissions:emissions.filter(e=>!native.some(n=>n.record.requestId===e.record.requestId)).length,receivedBatches:data.batches.length,verifiedAt:new Date().toISOString()};
}
