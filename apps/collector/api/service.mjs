...
function sanitize(e){let message;try{const m=JSON.parse(e.message);if(m.kind==='workshop-cookie-v1')message=JSON.stringify({kind:m.kind,run:m.run,eventId:m.eventId,cookieState:m.cookieState,...(Number.isInteger(m.slot)?{slot:m.slot}:{}),cookie:['fixture-A','fixture-B'].includes(m.cookie)?m.cookie:null});}catch{}
 ...
 const p=e.proxy;return {nativeCookieFieldPresent,excludedValuePresent:rawText.includes('NEVER_CAPTURE'),id:e.id,projectId:e.projectId,deploymentId:e.deploymentId,source:e.source,requestId:e.requestId,timestamp:e.timestamp,...(message?{message}:{}),...(p?{proxy:{path:p.path,referer:typeof p.referer==='string'?p.referer.split(/[?#]/)[0]:p.referer,userAgent:p.userAgent,vercelCache:p.vercelCache,statusCode:p.statusCode,cacheId:p.cacheId}}:{})};}
export default async function handler(req,res){
 ...
 try{
  ...
  if(op==='register'&&req.method==='POST'){
   ...
   if(cfg.mode==='browser'&&(!Number.isFinite(cfg.browserExpiresAt)||cfg.browserExpiresAt>Date.now()+610000||cfg.browserExpiresAt<Date.now()))return res.status(400).end();
  ...
  if(op==='origin'){
   const ticket=req.headers['x-fixture-ticket']||'',parts=ticket.split('.'),browser=ticket.startsWith('b.');
   const expires=browser?parts[1]:parts[0],sig=browser?parts[3]:parts[1];
   const signed=browser?`browser|${run}|${expires}|${parts[2]}`:`${run}|${expires}`;
   if(Number(expires)<Date.now()||!equal(sig,createHmac('sha256',process.env.FIXTURE_SECRET).update(signed).digest('hex')))return res.status(403).json({error:'invalid origin ticket'});
   if(browser){
    const [originExpires,eventId,originSig]=(req.headers['x-workshop-origin-ticket']||'').split('.');
    const consumed=await read(`runs/${run}/consumed/${parts[2]}.json`);
    if(!consumed||consumed.eventId!==eventId||originExpires!==expires||!equal(originSig,createHmac('sha256',process.env.FIXTURE_SECRET).update(`origin|${run}|${expires}|${eventId}`).digest('hex')))return res.status(403).json({error:'private origin authorization required'});
    try{await save(`runs/${run}/origin-authorizations/${eventId}.json`,{at:Date.now()});}catch{return res.status(409).json({error:'origin authorization already used'});}
   }
  ...
  if(op==='consume'&&req.method==='POST'){
   ...
   if(cfg.mode!=='browser'||Date.now()>cfg.browserExpiresAt||await read(`runs/${run}/closed.json`))return res.status(410).json({error:'Session expired or reset. Prepare a new session.'});
   if(!Number.isInteger(item.slot)||item.slot<1||item.slot>12||!/^[a-f0-9-]{36}$/.test(item.eventId||''))return res.status(400).end();
   const issued=await read(`runs/${run}/issued/${item.slot}.json`);
   if(!issued||issued.expires!==item.expires||Date.now()>issued.expires)return res.status(403).json({error:'Ticket expired. Get a fresh ticket.'});
   try{await save(`runs/${run}/consumed/${item.slot}.json`,{...issued,eventId:item.eventId,at:Date.now()});}catch(e){if(await read(`runs/${run}/consumed/${item.slot}.json`))return res.status(409).json({error:'Ticket already used. Get a fresh ticket for another request.'});throw e;}
   return res.json({accepted:true});
  }
  ...
  if(op.startsWith('browser-')){
   ...
   if(op==='browser-close'){await save(`runs/${run}/closed.json`,{at:Date.now()},true);return res.json({closed:true});}
   if(Date.now()>cfg.browserExpiresAt||await read(`runs/${run}/closed.json`))return res.status(410).json({error:'Session expired or reset. Prepare a new session; old evidence remains readable.'});
   if(input.deploymentId!==cfg.deploymentId)return res.status(409).json({error:'The request deployment changed. Reset and prepare a new session.'});
   if(op==='browser-join')return res.json({run,browserExpiresAt:cfg.browserExpiresAt,environment:env,deploymentId:cfg.deploymentId,revision:cfg.revision,contentPath:`/demo/${run}`,maximumRequests:12});
   if(op==='browser-ticket'){
    if(!['A','B','Missing','Invalid'].includes(input.expected))return res.status(400).json({error:'Select a cookie case first'});
    for(let slot=1;slot<=12;slot++){
     if(await read(`runs/${run}/issued/${slot}.json`))continue;
     const expires=Math.min(cfg.browserExpiresAt,Date.now()+(input.shortTicket?1000:60000));
     try{await save(`runs/${run}/issued/${slot}.json`,{slot,expected:input.expected,expires});}catch(e){if(await read(`runs/${run}/issued/${slot}.json`))continue;throw e;}
     const signature=createHmac('sha256',process.env.FIXTURE_SECRET).update(`browser|${run}|${expires}|${slot}`).digest('hex');
     return res.json({ticket:`b.${expires}.${slot}.${signature}`,slot,expires,remaining:12-slot});
    }
    return res.status(429).json({error:'All 12 request tickets have been allocated. Reset for a fresh session.'});
   }
  ...
  if(op==='evidence'){
   ...
   const result=cfg.mode==='browser'?analyzeBrowser({...data,consumed:await rows(`runs/${run}/consumed/`)}):analyze(data);
   return res.json({...result,...data,mode:cfg.mode||'hosted',browserExpiresAt:cfg.browserExpiresAt,collectorRevision:process.env.VERCEL_GIT_COMMIT_SHA,collectorSourceMappings:sourceMappings});
  }
  return res.status(404).end();
