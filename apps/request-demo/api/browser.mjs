...
export default async function handler(req,res){
 ...
 try{
  if(input.op==='prepare'){
   ...
   return res.json({run,capability,browserExpiresAt,collectorUrl:collector,environment,deploymentId:process.env.VERCEL_DEPLOYMENT_ID,revision:process.env.VERCEL_GIT_COMMIT_SHA,contentPath:`/demo/${run}`,maximumRequests:12});
  }
  if(!['ticket','join','close'].includes(input.op))return res.status(400).json({error:'Unknown operation'});
  return res.json(await call('browser-'+input.op,input.run,{expected:input.expected,shortTicket:input.shortTicket===true,deploymentId:process.env.VERCEL_DEPLOYMENT_ID},req.headers.authorization||''));
