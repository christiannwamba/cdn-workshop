...
const environment = process.env.WORKSHOP_ENV || process.env.PILOT_ENV;
export default async function handler(req, res) {
  ...
  if (req.headers.origin && req.headers.origin !== `https://${req.headers.host}`)
    return res
      .status(403)
      .json({ error: 'Open this lab as a top-level page on its request host' });
  const input = typeof req.body === 'string' ? JSON.parse(req.body) : req.body || {};
  const collector = process.env.COLLECTOR_URL;
  const call = async (op, run, data, authorization) => {
    const r = await fetch(
      `${collector}/api/service?op=${op}&run=${encodeURIComponent(run || '')}`,
      {
        method: 'POST',
    ...
    if (!r.ok) {
      const e = Error(value.error || `HTTP ${r.status}`);
      e.status = r.status;
      throw e;
    }
    return value;
  ...
  try {
    if (input.op === 'prepare') {
      ...
      const run = 'run-' + randomUUID(),
        ...
        browserExpiresAt = Date.now() + 600000;
      await call(
        'register',
        run,
        {
        ...
        `Bearer ${process.env.SERVICE_SECRET}`,
      );
      return res.json({
        sourceMappings,
        run,
    ...
    if (!['ticket', 'join', 'close'].includes(input.op))
    ...
    return res.json({
      ...(await call(
        ...
        input.run,
        {
          ...
          shortTicket: input.shortTicket === true,
          deploymentId: process.env.VERCEL_DEPLOYMENT_ID,
        },
