...
export function correlateReceipts(receipts,rows){
 ...
 const matched=accepted.filter(r=>rows.some(e=>e.slot===r.slot&&e.eventId===r.headers['x-workshop-event-id']&&e.joined));
 return {accepted:accepted.length,matched:matched.length,unique,complete:accepted.length>0&&unique&&matched.length===accepted.length};
}
