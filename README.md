...
Prepare a session, open Network with Disable cache **unchecked**, Set A → Send, Set B → Send, then refresh logs. Missing and invalid values are optional next steps. Reset closes the session and clears exact-path test cookies. Ten-minute sessions allow twelve one-use requests. The backend retains its existing session, expiry, signing and isolation behavior; advanced joining and regression automation are outside the primary teaching UI.
