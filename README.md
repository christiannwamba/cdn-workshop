...
## Real-browser R32 lab

Open the request deployment's `/lab.html` from the shared runbook. Use a top-level page so cookies belong to the host serving the tested content. Prepare a session, open Network with Disable cache **unchecked**, Set A → Send, Set B → Send, clear the selected cookie → Send, then set invalid → Send. Inspect the actual outgoing Cookie and the `x-vercel-cache`, `x-workshop-event-id`, and `x-workshop-fill-id` response headers. Run all executes those same cookie writes and fetches in the browser; `/api/run` remains the separate hosted regression driver.

Ten-minute sessions issue at most twelve one-use, sixty-second request tickets. Expired tickets can be replaced on the same run URL; an expired/closed session requires Reset and a fresh namespace. The optional one-second ticket control tests expiry quickly. Session keys remain in memory, are never URL parameters/localStorage entries, and can explicitly be revealed to join the same synthetic session from another isolated browser. Reset revokes the shared session and clears only that browser's exact-path test cookies.

The browser sets only synthetic `workshop_choice` and `workshop_unrelated` cookies, host-only with exact run path, Secure and SameSite=Strict. They are deliberately JS-accessible fixture data. Middleware reads the allowlisted value, then removes Cookie and Authorization headers before the origin rewrite. Browser tickets cannot call the origin directly: that hop needs a separate private, one-use authorization. Browser-mode ticket consumption adds a pre-cache collector round trip and private storage operation, separate from the content-origin counter. This is a teaching/security control, not a cost benchmark or production architecture recommendation.

The verifier uses authenticated Middleware-generated event IDs plus genuine signed emission/native requestId joins. Client labels are expectations, and client receipts are separately identified observations; neither can create a verified join. Manual partial progress stays PARTIAL. HTTP revalidation may reuse a browser body while contacting Vercel; require new delivered request IDs, not a cache label alone.

The central public profile chooses the request host to open. The lab uses that deployment's own generated public profile and server-configured collector; a browser-local override cannot redirect backend credentials or mix environments. Source links update from service revision metadata. See `evidence/browser/controls.md`, browser receipts, and redacted signed-delivery snapshots. Browser fingerprints, raw cookies, session keys and signing secrets are excluded from committed browser evidence.
