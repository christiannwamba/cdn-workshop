...
Open Network with Disable cache **unchecked**, start a fresh test, send the request with cookie A, then with cookie B, and refresh logs. Each send sets the cookie and fetches the same test URL. Starting a test does neither. Missing and invalid values are optional next steps. Reset closes the session and clears exact-path test cookies. Ten-minute sessions allow twelve one-use requests. The backend retains its existing session, expiry, signing and isolation behavior; advanced joining and regression automation are outside the primary teaching UI.
